    ...
    public override void DrawTab()
    {
        ...
        {  
            ...
            {  
                ...
                if (ImGui.Checkbox("启用战斗数据悬浮显示", ref combatDataOverlay))  
                {  
        ...
        if (ImGui.Checkbox("高亮显示当前任务相关的NPC", ref highlightNpc))  
        {  
        ...
        using (ImRaii.PushIndent())
        {
            ...
            ImGuiComponents.HelpMarker(
                "通常情况下，在使用 Questionable 运行副本时，AutoDuty 中配置的循环设置会被禁用，因为它们可能会导致问题（甚至把你的电脑关机）。");
        }
        ...
        using (ImRaii.PushIndent())
        {
            ...
            if (ImGui.Checkbox("插件加载时显示窗口", ref showWindowOnStart))
            {
