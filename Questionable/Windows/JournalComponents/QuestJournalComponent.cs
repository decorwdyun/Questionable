...
internal sealed class QuestJournalComponent
{
    ...
    public void DrawQuests()
    {
        ...
        if (ImGui.CollapsingHeader("说明", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            ImGui.BulletText("'已支持' 列出Questionable可以为你完成的任务");
            ImGui.BulletText("'已完成的' 列出你当前角色已完成的任务。");
        ...
        if (_filteredSections.Count > 0)
        {
            ...
            ImGui.TableSetupColumn("已支持", ImGuiTableColumnFlags.WidthFixed, 120 * ImGui.GetIO().FontGlobalScale);
