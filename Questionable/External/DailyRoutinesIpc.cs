...
internal sealed class DailyRoutinesIpc: IDisposable
{
    ...
    private readonly ICallGateSubscriber<string, bool?> _loadModule;
    private readonly ICallGateSubscriber<string, bool?> _unloadModule;
        ...
        )
    {
        ...
        _loadModule = pluginInterface.GetIpcSubscriber<string, bool?>("DailyRoutines.LoadModule");
        _unloadModule = pluginInterface.GetIpcSubscriber<string, bool?>("DailyRoutines.UnloadModule");
        ...
        try
        {
            if (_configuration.General.ConfigureDailyRoutines && hasActiveQuest)
            {
                ...
                foreach (var conflictingModule in _conflictingModules)
                {
                    ...
                    {
                        _unloadModule.InvokeFunc(conflictingModule);
    ...
    private void RecoverSuppressedModules()
    {
        if (IsDailyRoutinesEnabled && _suppressedModules.Count > 0)
        {
            ...
            {
                _loadModule.InvokeFunc(suppressedModule);
